...
What the frontend team builds against. All endpoints listed here are implemented on `main` under `/api/v2`.
...
**Latency:** the POST is synchronous and waits for extraction and delta apply, roughly 2 to 6 seconds with Gemini (instantaneous with the mock extractor). Show the user's message optimistically, render a thinking indicator, then drop the bot reply when the response arrives.
...
All endpoints are implemented on `main` under `/api/v2`. Run `make dev-mock` for a zero-setup environment (no DB, no API keys); set `EXTRACTOR_BACKEND=gemini` and supply `GEMINI_API_KEY` to use the real extractor. See the root `README.md` for the full quickstart.
